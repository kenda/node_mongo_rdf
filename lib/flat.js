...
var NodeMongoRdfFlat = exports.NodeMongoRdfFlat = function(db, host, port){
  ...
  function _flat_to_rdf_json(flats){
    rdf = {};
    for (flat in flats){
      rdf[flats[flat].subject] = {};
      for (p in flats[flat].predicates){
        if (flats[flat].predicates[p].uri !== undefined){
          rdf[flats[flat].subject][flats[flat].predicates[p].uri] = [];
          for (o in flats[flat].predicates[p].objects){
            rdf[flats[flat].subject][flats[flat].predicates[p].uri].push(flats[flat].predicates[p].objects[o]);
          }
        }
      }
    }
